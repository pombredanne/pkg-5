...
def executeCommand(command_line):
    command_line = 'PATH=$PATH:/usr/local/bin;' + command_line.replace("$", "\\$")
    error_code = subprocess.call([command_line], shell=True)
    if error_code != 0:
        raise BaseException(error_code)
